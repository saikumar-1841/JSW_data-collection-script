...
def compress_image(input_image_path, output_image_path, max_width, max_height, quality):
...
class JSWCameraClass(MasterCameraClass):
    def __init__(self, setup_name='', param_dict=''):
        try:
            ...
            self.compress_quality = 80  # Quality for compression
            self.max_width = 1920  # Maximum width for compression
            self.max_height = 1080  # Maximum height for compression
            self.init_camera()
            ...
            exc_type, exc_obj, exc_tb = sys.exc_info()
            connect.loginfo(
                f'[Camera] [{self.name}] Error in init method: {err.__str__()} - line no. : {exc_tb.tb_lineno}',
                level='error')

    def get_time_interval_folder(self):
        current_time = datetime.now()
        date_str = current_time.strftime('%b %d')
        hour_str = current_time.strftime('%b %d %I %p')
        time_interval_str = f"{current_time.strftime('%b %d %I%M')}_{(current_time + timedelta(minutes=15)).strftime('%I%M')}"
        return os.path.join(date_str, hour_str, time_interval_str)
    ...
    def acqFrames(self):
        try:
            ...
            try:
                counter = abs(((datetime.strptime((str(datetime.now())).split('.')[0],
                                                  self.time_format)) - datetime.strptime(temp,
            ...
            try:
                if connect.master_redis.get_val(key='record') == 'true' and (counter < 1200) and (
                        key_existence == True):
                    prefix = '/ext512/JSW/nvidia/record_imgs/record_pub_' + datetime.strftime(
                        datetime.now(tz_NY),
                        self.time_format_1)
                    area = self.area
                    ...
                    time_interval_folder = self.get_time_interval_folder()
                    full_path = os.path.join(prefix, area, time_interval_folder)
                    if not os.path.isdir(full_path):
                        os.makedirs(full_path)

                    # Save the compressed image
                    timestamp = datetime.now().strftime('%H%M%S%f')
                    compressed_img_name = f'{timestamp}.jpeg'
                    compressed_img_path = os.path.join(full_path, compressed_img_name)
                    compress_image(img, compressed_img_path, self.max_width, self.max_height,
                                   self.compress_quality)

            except Exception as e:
                connect.loginfo(f"[JSWCameraClass] [{self.name}] Error while processing frame: {e}",
                                level='error')
        except Exception as e:
            self.cam_health = False
            connect.loginfo("[JSWCameraClass] Exception occurred while acquiring frames: {}".format(e),
                            level='error')
        ...
        uid_left = str(uuid4())
        uid_right = str(uuid4())
        self.connect.master_redis.set_val(key=uid_left, val=enc_left.tobytes().hex(), ex=self.image_expiry_time)
        self.connect.master_redis.set_val(key=uid_right, val=enc_right.tobytes().hex(), ex=self.image_expiry_time)
        ...
        self.cam_health = True
        self.connect.loginfo(f'[Camera] [{self.name}] packet created')
        return res_packet
    except Exception as err:
        self.cam_health = False
        exc_type, exc_obj, exc_tb = sys.exc_info()
        self.connect.loginfo(
            f'[Camera] [{self.name}] Error in acqFrames method: {err.__str__()} - line no. : {exc_tb.tb_lineno}',
            level='error')
    ...
    def init_camera(self):
        try:
            ...
            # self.Cam_left.f.UserSetSelector.SetString("Default")
            # self.Cam_left.f.UserSetLoad.Execute()

            # self.Cam_right.f.UserSetSelector.SetString("Default")
            # self.Cam_right.f.UserSetLoad.Execute()
            ...
            # self.Cam_left.f.Gamma.Set(self.gamma)
            # self.Cam_right.f.Gamma.Set(self.gamma)
            ...
            exc_type, exc_obj, exc_tb = sys.exc_info()
            connect.loginfo(
                f'[Camera] [{self.name}] Error in init_camera method: {err.__str__()} - line no. : {exc_tb.tb_lineno}',
                level='error')
